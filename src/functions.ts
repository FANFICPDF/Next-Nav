  ...
  if (!!vscode.workspace.workspaceFolders) {
    ...
    try {
      ...
      const srcDir = entries.find(
        (entry) => entry.isDirectory() && entry.name === srcDirName
      );
      if (!srcDir) {
        return {
          status: 'error',
          message: `No ${srcDirName} directory found.`,
        };
      }
      ...
      const appDir = srcEntries.find(
        (entry) => entry.isDirectory() && entry.name === appDirName
      );
      if (!appDir) {
        return {
          status: 'error',
          message: `No ${appDirName} directory found in ${srcDirName}.`,
        };
      }
      ...
      return {
        status: 'error',
        message: 'Error reading workspace: ' + err.message,
      };
    }
  } else {
